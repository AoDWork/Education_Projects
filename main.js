...
{//006 Promise
    ...
    // all Промисы вернут результат который можем обработать через then. Этот метод служит для того что бы точно убедится что все
    //промисы выполнились. Например запрашиваем 4 картинки из разных серверов, и что бы одновременно их показать ждем пока 
    //все промисы выполнятся. Ориентируемся на промис который выполнится последним.
    Promise.all([test(1000), test(2000), test(3000)]).then(() => {
        console.log('All');
    });
    //Можно дописать catch для обработки ощибки
    
    // race  - этот метод начнет выполнятся как только выполнится самый первый промис из массива
